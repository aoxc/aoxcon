import { z } from 'zod';
import { AOXC_OKX_EXPLORER_URL, AOXC_OKX_TOKEN_URL, AOXC_TOKEN_ADDRESS } from '@/lib/network';

const intervalSchema = z.enum(['1m', '5m', '15m', '1h', '4h', '1d']);

const baseUrl = process.env.AOX_API_BASE_URL || 'https://api.aoxcore.com';
const marketSymbol = process.env.AOX_MARKET_SYMBOL || 'AOXCUSDT';

function withTimeout(ms: number): AbortSignal {
  return AbortSignal.timeout(ms);
}

export function normalizeTicker(payload: any) {
...
export async function fetchAoxTicker() {
  const res = await fetch(`${baseUrl}/api/v1/market/ticker?symbol=${marketSymbol}`, {
    headers: {
...
export const demoTicker = normalizeTicker({
  symbol: marketSymbol,
  lastPrice: 0.0614,
