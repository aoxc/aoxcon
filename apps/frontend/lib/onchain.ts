...
import { AOXC_TOKEN_ADDRESS, getRpcFallbackOrder } from './network';
...
const aoxMainnet = defineChain({
  ...
  rpcUrls: {
    default: { http: getRpcFallbackOrder('main') },
    public: { http: getRpcFallbackOrder('main') },
  },
});

const client = createPublicClient({
  chain: aoxMainnet,
  transport: fallback(getRpcFallbackOrder('main').map((url) => http(url))),
});
