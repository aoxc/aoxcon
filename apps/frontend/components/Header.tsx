...
export default function Header({ onMenuToggle }: HeaderProps) {
  const { state, connectWallet, disconnectWallet } = useDemo();
      ...
      <header className="sticky top-0 h-20 border-b border-white/10 bg-black/40 backdrop-blur-xl z-40 px-4 md:px-8 grid grid-cols-3 items-center">
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <div className="relative">
            ...
            >
              <div className="w-1.5 h-1.5 rounded-full bg-aox-green" />
            ...
            <AnimatePresence>
              {isNetworkDropdownOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 5 }}
                  className="absolute top-full mt-2 right-0 w-72 bg-[#0D1117] border border-white/10 rounded-lg p-3 shadow-2xl z-50"
                >
                  <p className="text-xs font-semibold">{state.networkProfile.label}</p>
                  <p className="text-[10px] text-white/40 mt-1">RPC: {state.networkProfile.rpcEndpoints[0]}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
